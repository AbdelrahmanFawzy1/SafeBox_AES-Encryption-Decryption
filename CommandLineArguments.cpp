...
CommandLineArguments::CommandLineArguments()
{
    ...
    memset(error_string,0,ERROR_STRING_MAX_SIZE);
        memset(tree_file,0,COMMAND_LINE_ARGUMENT_MAX_SIZE);//...
    ...
    commands["List"] = new SafeBoxList(); // Another two classes the list and delete to make it work 
    commands["Delete"] = new SafeBoxDelete();
}
    ...
    else
    {
        for ( uint8_t i = 1 ; i < argc ; i +=2)
        {
            if ( strcmp(argv[i],"--input-file") == 0)
                ...
                else if (strcmp( argv[i], "--tree-file")==0) // Here to get the tree file
               strncpy(tree_file,argv[i+1], COMMAND_LINE_ARGUMENT_MAX_SIZE-1);//..
        
            else if (strcmp(argv[i], "--type")==0) // Here to know whether it is ascending or descending 
              {
                    if (strcmp(argv[i+1],"asc") == 0) flag = true;
                    else flag = false; 
              }  
            else
            {
