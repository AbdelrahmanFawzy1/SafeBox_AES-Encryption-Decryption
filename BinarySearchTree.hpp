...
template <class NODE, typename ITEM> void BinarySearchTree<NODE, ITEM>::traverseASC(NODE * n,fstream &index_file)
{
	if (n == NULL) return;
//cout << " HERE"<<endl; 
	traverseASC(n->getLeft(),index_file);
	if(n->getItem().flag!=true){index_file<<&(n->getItem())<<endl;
cout << " WELCOME" << endl; 
}
	traverseASC(n->getRight(),index_file);
}
...
template <class NODE, typename ITEM> void BinarySearchTree<NODE, ITEM>::traverseASC(fstream &index)
{
	traverseASC(root,index);
...
cout << " AYMAN " << endl; 
	...
	if ( next == parent ) return next; } return NULL;
...
if ( flag ){
if (n == NULL) return;
std::cout<<"AKKKKKKKKKKK"<<endl;
	process(n->getLeft(),[](NODE* n)->void{  cout<<n->getItem().filename << "," << n->getItem().blocksize << ","<< n->getItem().size_in_blocks<<"," << n->getItem().size_in_bytes<< "," <<n->getItem().shreds
            << "," <<n->getItem().hidden_name << endl;},flag);
    process_node(n);
	process(n->getRight(), [](NODE* n)->void{  cout<<n->getItem().filename << "," << n->getItem().blocksize << ","<< n->getItem().size_in_blocks<<"," << n->getItem().size_in_bytes<< "," <<n->getItem().shreds
            << "," <<n->getItem().hidden_name << endl;},flag);
...
cout << " OK " << endl;
 cout<<n->getItem().filename << "," << n->getItem().blocksize << ","<< n->getItem().size_in_blocks<<"," << n->getItem().size_in_bytes<< "," <<n->getItem().shreds
            << "," <<n->getItem().hidden_name << endl;
cout << " OK " << endl; },flag);
cout << " WOW" << endl; 
    process_node(n);
cout << " WOW" << endl; 
...
template <class NODE, typename ITEM> void BinarySearchTree<NODE, ITEM>::process(std::function <void (NODE *)> process_node, bool flag)
{	
        NODE* N = getRoot();
process(N, process_node, flag);
}
