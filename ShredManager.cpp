...
// Since this function depends on one shred only, so I will call it three times if I requested three shreds. 
  ...
  CryptoPP::byte iv[ CryptoPP::AES::BLOCKSIZE];
    ...
    if ( f.is_open())
    {
        f.read (reinterpret_cast<char*>(iv),sizeof(iv)); //Write it in the file respecting to what the doctor wrote.
   ...
   // I need to call the EncryptShredThread function to each thread. Here is why I need to use the for loop.
        ...
        fname += fname.find('.')+(i+1+'A');// Changing the names of the shred files not to overwrite
        fname += ".txt"; 
       std:: cout << iv <<std::endl;
        char pname = ('A'+i);
        if( truncate) // This if condition to enter into the first one.
shreds[i]= new EncryptShredThread(p_fileSpooler,key_file_name,iv_file_name,(char*)(fname.c_str()),block_size,&lottery,&multiHeadQueue,pname,truncate);else
      shreds[i]= new EncryptShredThread(p_fileSpooler,(key_file_name),iv_file_name,((char*)(fname.c_str())),(block_size+16)&~15,&lottery,&multiHeadQueue,pname,truncate);
